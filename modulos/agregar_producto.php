...
<form method="post" action="" enctype="multipart/form-data" style="font-family: 'Orbitron', 'Arial';">
	<label style="color: #fff; font-size: 16px; ">Datos del Videojuego <span class="icon-rocket"></span></label>
	<br>
	<br>
	...
	<label style="color: #fff; font-size: 16px;">Imagen del producto <span class="icon-image"></span> </label>
	<br>
	<br>
	<div class="form-group">
		...
		<select name="categoria" required class="form-control">
			...
			<?php
				...
				while($r=mysqli_fetch_array($q)){
					?>
						<option value="<?=$r['id_categoria']?>"><?=$r['categoria']?></option>
					<?php
				}
			?>
		</select>
...
<table class="table table-striped" style="font-family: 'Orbitron', 'Arial'; background-color: gray; ">
	...
	<?php
		...
		while($rp=mysqli_fetch_array($prod)){
			...
			$cat = $mysqli->query("SELECT * FROM categorias WHERE id_categoria = '".$rp['id_categoria']."'");
